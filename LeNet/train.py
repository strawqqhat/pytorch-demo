...
# def imshow(img):
#     img = img / 2 + 0.5  # output = (input-0.5)/0.5 => input=output/2+0.5
#     npimg = img.numpy()
#     plt.imshow(np.transpose(npimg, (1,2,0)))  # (channel,height,width)=>(height,width,channel)
#     plt.show()
#
# # print label
# print(' '.join('%5s' %classes[test_label[j]] for j in range(111)))
# # show images
# imshow(torchvision.utils.make_grid(test_image))
net = LeNet()
loss_function = nn.CrossEntropyLoss()
optimizer = optim.Adam(net.parameters(), lr=0.001)  # 优化net的所有参数
